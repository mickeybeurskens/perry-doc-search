...
def display_instructions(request_manager):
    ...
    st.info("__🔥 Tip:__ We'll give you some prompts to get started, but it's useful start experimenting with them yourself."
    ...
    st.write("First, we need  documents to work with. You can upload PDF files going to the `Documents` page."
    ...
    st.info("Filling in the __description__ for each document is important! It will be used by the agent to understand the contents and answer your questions.")

    st.subheader("2. Create a conversation 🤖")
    st.write("Now that we have some documents to work with, we can create a conversation."
    ...
    st.info("There are multiple agents to choose from. Each agent handles documents differently. Experiment to see which one works best for you!")

    st.subheader("3. Start chatting with your agent! 💬")
    st.write("Now that you've created a conversation, you can start chatting with your agent on the `Chat` page!"
    ...
    st.write("That's it! You're ready to start working with your documents. If you have any questions, feel free to reach out to us at [🔥Forge Fire AI Engineering](forgefire.dev).")

    st.subheader("4. Bonus: Prompt Library 📚")
    st.write("If you're not sure what to ask your agent, use one of the following prompts to get started!")
    st.write("__Summarize a document:__")
    st.write("Use this style of prompting to summarize a document or go through documents quickly.")
    st.code("""
    ...
    st.write("__Q&A:__")
    st.write("Use this style of prompting to ask questions and get answers in a specific format by proving an example.")
    st.code("""
